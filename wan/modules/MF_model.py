...
class LoRALinear(nn.Module):
    def __init__(self, in_dim, out_dim, r=256, alpha=512.0):
        super().__init__()
        self.r = r
        self.alpha = alpha
        self.in_dim = in_dim
...
class ConditionCrossAttention(ConditionSelfAttention):
    def forward(self, x, c, grid_sizes, freqs):
        r"""
        Args:
            x(input)(Tensor): Shape [B, L, C]
            c(condition)(Tensor): Shape [B, L, C]
        """
        b, s, n,d = *x.shape[:2], self.num_heads, self.head_dim

        q = self.norm_q(self.q(x)).view(b,-1,n,d)
        k = self.norm_k(self.k(c)).view(b,-1,n,d)
        v = self.v(c).view(b,-1,n,d)
...
class WanAttentionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.cross_attn = WanT2VCrossAttention(dim, num_heads, qk_norm, eps, r, alpha)
...
class ConditionFirstBlock(nn.Module):
    def __init__(self,
        ...
        # layers
        self.norm_c = WanLayerNorm(dim, eps)
        self.self_attn_c = ConditionSelfAttention(dim, num_heads, qk_norm, eps)
    ...
    def forward(self, x, c, e, seq_lens, grid_sizes, freqs):
        r"""
        Args:
            x(Tensor): Shape [B, L, C]
            c(Tensor): Shape [B, L, C]
            e(Tensor): Shape [B, 6, C]
            seq_lens(Tensor): Shape [B], length of each sequence in batch
            grid_sizes(Tensor): Shape [B, 3], the second dimension contains (F, H, W)
            freqs(Tensor): Rope freqs, shape [1024, C / num_heads / 2]
        ...
        assert e.dtype == torch.float32
        with amp.autocast("cuda", dtype=torch.float32):
            modulation = self.modulation.expand(e.shape[0], 9, -1)
            modulation = torch.cat([modulation[:,:3],modulation[:,3:] + e], dim = 1).chunk(9, dim =1)
            # e = (self.modulation + e).chunk(6, dim=1)
        assert modulation[0].dtype == torch.float32

        # self-attn for conditions
        y_c = self.self_attn_c(self.norm_c(c).float() * (1 + modulation[1]) + modulation[0], seq_lens, grid_sizes,
            freqs)
        with amp.autocast("cuda", dtype=torch.float32):
            c = c + y_c * modulation[2]
        
        # cross-attn for x attending (x,c1,c2)
        y = self.cross_attn(self.norm3(x).float() * (1 + modulation[4]) + modulation[3], c, grid_sizes, freqs)
        with amp.autocast("cuda", dtype=torch.float32):
            x = x + y * modulation[5]
        y = self.ffn(self.norm4(x).float() * (1 + modulation[7]) + modulation[6])
        with amp.autocast("cuda", dtype=torch.float32):
            x = x + y * modulation[8]
...
class ConditionAttentionBlock(nn.Module):
    ...
    def forward(self, x, c, e, grid_sizes, freqs):
        assert e.dtype == torch.float32
        with amp.autocast("cuda", dtype=torch.float32):
            e = (self.modulation + e).chunk(6, dim=1)
        assert e[0].dtype == torch.float32
        y = self.cross_attn(self.norm1(x).float() * (1 + e[1]) + e[0], c, grid_sizes, freqs)
        with amp.autocast("cuda", dtype=torch.float32):
            x = x + y * e[2]
...
class Head(nn.Module):
    ...
    def __init__(self, dim, out_dim, patch_size, eps=1e-6, r=256, alpha=512.0):
        super().__init__()
        self.dim = dim
        self.out_dim = out_dim
...
class TimeAttentionLayer(nn.Module):
    def __init__(self, dim, num_heads, qk_norm = True, eps = 1e-6):
        ...
        self.norm_q = WanRMSNorm(dim, eps=eps) if qk_norm else nn.Identity()
        self.norm_k = WanRMSNorm(dim, eps=eps) if qk_norm else nn.Identity()
        self.norm_attn = WanLayerNorm(dim, eps, True)
        self.norm_ffn = WanLayerNorm(dim * 2, eps, True)
        self.ffn = nn.Sequential(nn.Linear(dim * 2, dim))

    def forward(self, x):
        r"""
        Args:
            x(Tensor of two timesteps): Shape [B, 2, num_heads, C / num_heads]
            seq_lens(Tensor): Shape [B]
        """
        b, s, n, d = *x.shape[:2], self.num_heads, self.head_dim
        def qkv_fn(x):
        ...
        # output
        x = x.flatten(2)
        x = self.o(x).view(b, s*n*d)
        x = self.ffn(self.norm_ffn(x))

        return x

    
class MFModel(ModelMixin):
    r"""
    Generative model with dinov3 embeddings as strong condition while maintain context_null as weak condition.
    """
    ...
    def __init__(self,
        ...
        self.time_combiner = TimeAttentionLayer(dim, num_heads, qk_norm, eps)
        ...
        for i in range(num_layers):
            blocks.append(WanAttentionBlock(dim, ffn_dim, num_heads, qk_norm, cross_attn_norm, eps, r, alpha))
            if (i + 1) % 4 == 0:
    ...
    def forward(
        ...
        Args:
            ...
            c (Tensor):
                Dinov3 Visual Embeddings, each with same shape as patchified x_latent.
            t (Tensor):
        ...
        dino_embedding = c                     # tensor (B, T*H'*W', C_dino), H' = H/2, W'= W/2
        ...
        # time embeddings
        with amp.autocast("cuda", dtype=torch.float32):
            t1, t2 = sinusoidal_embedding_1d(self.freq_dim, t1).float(), sinusoidal_embedding_1d(self.freq_dim, t2).float()
            e01, e02 = self.time_embedding(t1), self.time_embedding(t2)
            e = self.time_combiner(torch.stack([e01, e02], dim=1))
            e0 = self.time_projection(e).unflatten(1, (6, self.dim))
            assert e01.dtype == torch.float32 and e02.dtype == torch.float32 and e.dtype == torch.float32 and e0.dtype == torch.float32
        ...
        x, c = self.first_layer(x, dino_embedding, e0, seq_lens, grid_sizes, self.freqs)
        ...
        for i in range(self.num_layers):
            ...
            if (i + 1) % 4 == 0:
                x = next(iter_blocks)(x, c, e0, grid_sizes, self.freqs)
        ...
        return torch.stack([u.float() for u in x])          ## Here we generate directly global tensor instead of list of tensors.
