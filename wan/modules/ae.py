...
class TimeHead(nn.Module):
    def __init__(self, c_in, c_out):
        super().__init__()
        self.proj1 = nn.Linear(c_in, c_out)
        self.proj2 = nn.Linear(c_out, c_out)
    ...
    def forward(self, x):
        ...
        # 3. 投影 + 激活
        x = F.silu(self.proj1(x)) # [B, T, C_out]
        
        # 4. 时间平均 (注意现在的 T 是 dim=1)
        x = x.mean(dim=1)        # [B, C_out]
        
        # 5. 输出投影
        x = self.proj2(x)        # [B, C_out]
        return x
...
class DistributionShifter(nn.Module):
    def __init__(self,
        ...
        self.time_head = TimeHead(z_dim * 2, time_range).to(device)
