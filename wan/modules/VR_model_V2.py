...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, lq, seq_lens, grid_sizes, freqs):
        r"""
        Args:
            x(Tensor): Shape [B, L, num_heads, C / num_heads]
            lq(Tensor): Shape [B, L, num_heads, C / num_heads]
            seq_lens(Tensor): Shape [B] int == 2*L
            grid_sizes(Tensor): Shape [B, 3], the second dimension contains (F, H, W)
            freqs(Tensor): Rope freqs, shape [1024, C / num_heads / 2]
        """
        b, s, n, d = *x.shape[:2], self.num_heads, self.head_dim
        
        total = torch.cat([x,lq], dim=1)

        q = self.norm_q(self.q(x) + self.qlora(x)).view(b, -1, n, d)
        k = self.norm_k(self.k(total) + self.klora(total)).view(b, -1, n, d)
        v = (self.v(total) + self.vlora(total)).view(b, -1, n, d)

        q = rope_apply(q, grid_sizes, freqs)
        k1 = rope_apply(k[:, :s], grid_sizes, freqs)
        k2 = rope_apply(k[:, s:], grid_sizes, freqs)
...
class WanT2VCrossAttention(nn.Module):
    def __init__(self, dim, num_heads, qk_norm=True, eps=1e-6, r = 256, 
                 alpha = 512.0):
        assert dim % num_heads == 0
        super().__init__()
        self.dim = dim
...
class WanFirstBlock(nn.Module):
    ...
    def forward(self, lq, seq_lens, grid_sizes, freqs):
        with amp.autocast("cuda", dtype=torch.float32):
            modulation = self.modulation.chunk(6, dim=1)
        assert modulation[0].dtype == torch.float32
        seq_lens = torch.stack([u // 2 for u in seq_lens], dim = 0)
        b,s,n,d = *lq.shape[:2], self.num_heads, self.head_dim
        assert s == seq_lens[0], "Seq_lens should be the same as shape[1] of lq."

        y = self.norm1(lq).float() * (1 + modulation[1]) + modulation[0]
        q = self.norm_q(self.q(y)).view(b, s, n, d)
        k = self.norm_k(self.k(y)).view(b, s, n, d)
        v = self.v(y).view(b, s, n, d)
        ...
        y = self.o(y)
        with amp.autocast("cuda", dtype=torch.float32):
            lq = lq + y * modulation[2]
        y = self.norm2(lq).float() * (1 + modulation[4]) + modulation[3]
        y = self.ffn(y)
        with amp.autocast("cuda", dtype=torch.float32):
            lq = lq + y * modulation[5]
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        z = self.norm1(torch.cat([x,lq], dim = 1)).float() * (1 + e[1]) + e[0]
        y, lq = z[:, :s], z[:, s:]

        # self-attention
        y = self.self_attn(y, lq, seq_lens, grid_sizes, freqs)
...
class Head(nn.Module):
    ...
    def __init__(self, dim, out_dim, patch_size, eps=1e-6, r=256,
                 alpha=512.0):
        super().__init__()
        self.dim = dim
        self.out_dim = out_dim
...
class VRModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.first_layer = WanFirstBlock(dim, ffn_dim, num_heads, qk_norm, eps)
        # blocks
    ...
    def forward(
        ...
        Args:
            ...
            lq (List[Tensor]):
                List of input low-quality video tensors, each with shape [C_in, F, H, W]
            t (Tensor):
        ...
        lq = [self.patch_embedding(u.unsqueeze(0)) for u in lq]
            
        grid_sizes = torch.stack(
            [torch.tensor(u.shape[2:], dtype=torch.long) for u in x])      # B, T', H', W' 
        
        x = [u.flatten(2).transpose(1, 2) for u in x]              # list of tensor with (1, T*H/2*W/2, dim)
        lq = [u.flatten(2).transpose(1, 2) for u in lq]   
        seq_lens = torch.tensor([u.size(1) for u in x], dtype=torch.long) + torch.tensor([u.size(1) for u in lq], dtype=torch.long)  # list of tensor [B,] int == L*2
        ...
        # seq_lens = torch.tensor([u.size(1) for u in lq], dtype=torch.long)
        # assert seq_lens.max() <= seq_len
        # lq = torch.cat([                                                           # B, L, C
        #     torch.cat([u, u.new_zeros(1, seq_len - u.size(1), u.size(2))],
        #               dim=1) for u in lq
        # ])
        x, lq = torch.cat(x), torch.cat(lq)                                      # B, L, C
        ...
        lq = self.first_layer(lq, seq_lens, grid_sizes, self.freqs)
        for block in self.blocks:
            x = block(x, lq, **kwargs)

        # head
        x = self.head(x, e)                            # x = [B, C, T, H, W]
